...
$("#addpermissionbtn").click(function(e){
    ...
    $.post('addpermission/', permissiondata, function(resultdata) {
        ...
        if (!(resultdata['error'])) {
            $("#permissions tbody").append('<tr id="permission' + permission_id + '"><td><input type="hidden" name="permission" value="' + permission_id + '" /><input type="hidden" name="name' + permission_id + '" value="' + name + '" />' + name + '</td><td><input type="hidden" name="method' + permission_id + '" value="' + method + '" />' + method + '</td><td>' + value + '</td><input type="hidden" name="value' + permission_id + '" value="' + value + '" /></td><td><a href="" data-confirm="Are you sure you want to delete this permission?" data-permission-id="' + permission_id + '"><i class="bi bi-trash"></i></td></tr>');
            var _m = bootstrap.Modal.getInstance(document.querySelector('#addPermission')); if (_m) { _m.hide(); }
        } else {
...
$("#addpermissionSetbtn").click(function(e){
    ...
    $(permissions[name]).each(function(i, permissiondata) {
        $.post('addpermission/', permissiondata, function(resultdata) {
            ...
            if (!(resultdata['error'])) {
                successcount++;
                $("#permissions tbody").append('<tr id="permission' + permission_id + '"><td><input type="hidden" name="permission" value="' + permission_id + '" /><input type="hidden" name="name' + permission_id + '" value="' + permissiondata['permission_name'] + '" />' + permissiondata['permission_name'] + '</td><td><input type="hidden" name="method' + permission_id + '" value="' + permissiondata['permission_method'] + '" />' + permissiondata['permission_method'] + '</td><td>' + permissiondata['permission_value'] + '</td><input type="hidden" name="value' + permission_id + '" value="' + permissiondata['permission_value'] + '" /></td><td><a href="" data-confirm="Are you sure you want to delete this permission?" data-permission-id="' + permission_id + '"><i class="bi bi-trash"></i></td></tr>');
            }
        });
    }).promise()
      .done( function() {
        var _m = bootstrap.Modal.getInstance(document.querySelector('#addPermissionsSet')); if (_m) { _m.hide(); }
    });
});
