...
def setup_func():
    with db.session.begin():
        tests = iatidq.models.Test.query.filter_by(test_group=TEST_GROUP).all()
        for test in tests:
...
def check_against_files(test_str):
    ...
    data_files = [("good.xml", True), ("bad.xml", False)]
...
def test_checks_from_csv():
    filename = os.path.join(os.path.dirname(__file__), "activity_tests.csv")
