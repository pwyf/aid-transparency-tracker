...
from beta.infotest import (
    calc_networked_data_3_activity_score,
    process_orgid,
    process_publishers,
    process_sector,
)
...
class TestIndicatorNetworkedData:
    ...
    def flask_app(self):
        ...
        app.config.from_pyfile(os.path.join("resources", "mock_flask_app_config.py"))
        return app
    ...
    def test_activity_level_score_transactions_w_names(
        self, org_id_prefixes, publishers_list, xm_dac_codes
    ):
        ...
        tree = etree.parse(
            os.path.join("unittests", "artefacts", "xml", "test_01_be-dgd-2021_1.xml")
        )
        ...
        for idx, activity in enumerate(activities):
            ...
            explanation, score = calc_networked_data_3_activity_score(
                org, transactions, org_id_prefixes, publishers_list, xm_dac_codes
            )

            assert score == expected_results[idx]

    def test_activity_level_score_transactions_w_refs(
        self, org_id_prefixes, publishers_list, xm_dac_codes
    ):
        ...
        tree = etree.parse(
            os.path.join("unittests", "artefacts", "xml", "test_02_be-dgd-2021_1.xml")
        )
    ...
    def test_activity_level_score_excludes_self_ref(
        self, org_id_prefixes, publishers_list, xm_dac_codes
    ):
        ...
        expected_results = [
            ("score: 3 (passing transactions) / 3 (total transactions assessed)", 1),
        ...
        tree = etree.parse(
            os.path.join(
                "unittests", "artefacts", "xml", "test_03-be-dgd-excludes-self-refs.xml"
            )
        )
