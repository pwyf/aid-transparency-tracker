...
def __setup_organisations(pkg_id):
    org_data = [
        ("UK, DFID", "GB-1", """participating-org[@role="Extending"][@ref="GB-1"]"""),
        (
            "World Bank",
            "44002",
    ...
    for name, code, cond in org_data:
        organisation = iatidq.dqorganisations.addOrganisation(
            {"organisation_name": name, "organisation_code": code}
        )
...
def get_packagegroups_by_name(name):
    return models.PackageGroup.query.filter(models.PackageGroup.name == name).all()


def get_packages_by_name(name):
    return models.Package.query.filter(models.Package.package_name == name).all()
...
def create_aggregation_types(options):
    ...
    all_ag = iatidq.dqaggregationtypes.addAggregationType(
    ...
    iatidq.dqaggregationtypes.addAggregationType(
        {
            "name": "Current data",
            "description": "",
...
def _test_example_tests(publisher, country):
    ...
    iatidq.dqimporttests.importTestsFromFile(
        "tests/sample_tests.csv", test_level.ACTIVITY
    )

    print("Importing indicators")
    iatidq.dqindicators.importIndicatorsFromFile(
        "test_pwyf2013", "tests/sample_tests.csv"
    )

    iatidq.dqindicators.importIndicatorDescriptionsFromFile(
        "2018index", "tests/indicators.csv"
    )
    ...
    assert iatidq.test_queue.check_file(
        test_functions, codelists, xml_filename, runtime.id, pkg.id
    )
    ...
    expected_test_ids = [
        i.id
        for i in models.Test.query.filter(models.Test.name.in_(resultful_tests)).all()
    ]
...
@nose.with_setup(setup_func, teardown_func)
@pytest.mark.parametrize("publisher, country", [("worldbank", "789"), ("dfid", "ph")])
